...
class Teen_Application
	def initialize
		...
		#generate applicaiton id. Used to log back into application at later time.
	def application_id
		@applicationid = rand(36**8).to_s(36).upcase
		puts "Your application id is #{@applicationid}"
	end

		#If already started an application, then enter their application id.
	...
	#search for programs based on content area and/or model type
	def search
	...
	#displays program options that fit applicant's search criteria and eligibility
	def program_options
	...
	private #exisiting applicationid
	def appid
		appid = "E2C2R48S"
	end

def data_collect #store data on firebase
